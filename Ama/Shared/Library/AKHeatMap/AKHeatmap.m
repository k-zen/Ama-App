                       ...
                       weights:(NSArray *)weights
{
    ...
    return [AKHeatMap heatMapWithRect:mapView.frame boost:boost points:points weights:weights];
}
...
+ (UIImage *)heatMapWithRect:(CGRect)rect
                     ...
                     weights:(NSArray *)weights
{
    return [AKHeatMap heatMapWithRect:rect
                                boost:boost
                               points:points
                              weights:weights
             ...
             groupingEnabled:(BOOL)groupingEnabled
{
    ...
    float peaksRemovaAKactor = 0.4; // Should be from 0 (no peaks removal) to 1 (peaks are completely lowered to zero)
    
    // Validate arguments
    if (points == nil || rect.size.width <= 0 || rect.size.height <= 0 || (weights != nil && [points count] != [weights count])) {
        NSLog(@"AKHeatMap: heatMapWithRect: incorrect arguments");
        return nil;
    }
    ...
    for (NSValue* pointValue in points) {
        ...
        if (point_x[i] < 0 - radius || point_y[i] < 0 - radius || point_x[i] >= rect.size.width + radius || point_y[i] >= rect.size.height + radius) {
            points_num--;
            j++;
            ...
            if (point_weight_percent[i]> 0) {
                for (j = i + 1; j < points_num; j++) {
                    if (point_weight_percent[j]> 0) {
                        ...
                        float K1 = 1 - peaksRemovaAKactor;
                        float K2 = peaksRemovaAKactor;
