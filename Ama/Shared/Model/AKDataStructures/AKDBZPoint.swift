...
class AKDBZPoint: NSObject {
    // MARK: Properties
    let debug = false
    let center: GeoCoordinate
    let intensity: DBZIntensity
    let mapRect: MKMapRect
    
    init(center: GeoCoordinate, intensity: DBZIntensity) {
        self.center = center
        ...
        //
        let pointRadius = GlobalConstants.AKDBZSize
        
        let pointA = MKMapPointForCoordinate(Func.AKLocationWithBearing(bearing: (3 * Double.pi) / 4, distanceMeters: pointRadius, origin: center))
        if self.debug {
            NSLog("=> INFO: POINT.A(x:%f,y:%f)", pointA.x, pointA.y)
        }
        
        let pointB = MKMapPointForCoordinate(Func.AKLocationWithBearing(bearing: (7 * Double.pi) / 4, distanceMeters: pointRadius, origin: center))
        if self.debug {
